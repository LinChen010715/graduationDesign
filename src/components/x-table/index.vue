...
// TODO: 持续优化
import { CustomPageTableForm, ElPaginationData, XTableAPI, XTableOptions } from '@/interface/table';
...
async function loadData(queryData?: CustomPageTableForm): Promise<boolean> {
    ...
    const res = await props.api.get(
        Object.assign(
            {
                [props.tableOptions.queryCurrentPageKey || 'page']: paginationData.currentPage,
                [props.tableOptions.queryPageSizeKey || 'limit']: paginationData.pageSize,
            },
    ...
    intersectionBy(tableData.value, checkedRows.value, props.rowKey).forEach((item) => {
        tableRef.value?.toggleRowSelection(item, true);
    });

    // 分页数据
    paginationData.total = res[props.tableOptions.returnTotalKey || 'total'] || 0;
    paginationData.currentPage = Number(res[props.tableOptions.returnCurrentPageKey || 'current']);
